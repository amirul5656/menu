...
# Function to add domain or subdomain
add_domain() {
    clear
    draw_box_title "Add Domain/Subdomain"
    ...
    read -p "Enter new domain or subdomain (example: sub.example.com): " NEW_DOMAIN
    read -p "Enter database name for new domain: " NEW_DB_NAME
    read -p "Enter database user for new domain: " NEW_DB_USER
    read -sp "Enter database user password for new domain: " NEW_DB_PASS
    echo
    read -sp "Enter MariaDB root password: " MARIADB_ROOT_PASSWORD
    echo

    DOC_ROOT="/var/www/html/$NEW_DOMAIN"
    NGINX_CONF="/etc/nginx/sites-available/$NEW_DOMAIN"
    NGINX_LINK="/etc/nginx/sites-enabled/$NEW_DOMAIN"

    draw_line
    echo "Adding new domain/subdomain..."
...
EOF
    ...
    echo "Domain/Subdomain $NEW_DOMAIN added successfully with database $NEW_DB_NAME."
    draw_line
    read -p "Press enter to continue..."
}

# Function to remove WordPress and its related applications
remove_all_apps() {
    clear
    draw_box_title "Remove All Applications"
    ...
    read -p "Enter domain name to remove (example: example.com): " DOMAIN
    read -p "Enter database name: " DB_NAME
    read -p "Enter database user: " DB_USER
    read -sp "Enter MariaDB root password: " MARIADB_ROOT_PASSWORD
    echo
    ...
    if [ -d "$DOC_ROOT" ]; then
        rm -rf $DOC_ROOT
        echo "Removed WordPress files from $DOC_ROOT."
    else
        echo "No WordPress installation found at $DOC_ROOT."
    fi
...
EOF
    ...
    # Remove Nginx, PHP, and other applications
    echo "Removing Nginx, PHP, and other applications..."
    apt-get purge -y nginx php8.3-fpm php8.3-mysql php8.3-curl php8.3-gd php8.3-mbstring php8.3-xml php8.3-xmlrpc php8.3-soap php8.3-intl php8.3-zip unzip mariadb-server
...
show_menu() {
    while true; do
        ...
        echo -e "${BLUE}[03]${NC} Add Domain/Subdomain   ${BLUE}[08]${NC} Disk Usage"
        echo -e "${BLUE}[04]${NC} Remove All Applications ${BLUE}[09]${NC} Process Monitor"
        echo -e "${BLUE}[05]${NC} Service Management     ${BLUE}[10]${NC} Exit"
        draw_line
        echo
